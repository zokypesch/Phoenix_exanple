defmodule MyApp.AuthTest do
  ...
  describe "users" do
    ...
    test "list_users/0 returns all users" do #changes by triadi
      user = user_fixture()
      assert Auth.list_users() == [%User{user | password: nil}]
    end

    test "get_user!/1 returns the user with given id" do #changes by triadi
      user = user_fixture()
      assert Auth.get_user!(user.id) ==  %User{user | password: nil}
    end

    test "create_user/1 with valid data creates a user" do #changes by triadi
    ...
    test "update_user/2 with valid data updates the user" do #changes by triadi
      user = user_fixture()
    ...
    test "update_user/2 with invalid data returns error changeset" do #changes by triadi
      user = user_fixture()
